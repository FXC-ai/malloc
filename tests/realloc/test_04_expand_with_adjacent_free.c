...
static void recap(void *old_ptr, void *new_ptr)
{
    ...
    if (new_ptr == 0)
        ...
        { P(C_MAGENTA); P("  ➜ Réalloc SANS déplacement (fusion avec bloc libre adjacent ?)\n"); P(C_RESET); }
    else
...
/* === Test 4 : Realloc vers plus grand avec bloc suivant free assez grand (16 -> 64) ===
   Setup : ptr1=16, ptr2=16 (sera libéré), ptr3=128. On agrandit ptr1 en 64. */
int main(void)
{
    ...
    banner("TEST 4 — Agrandissement avec bloc suivant libre suffisant",
           "Scenario: malloc(16), malloc(16), malloc(128) → free(ptr2) → realloc(ptr1, 64)");
    ...
    /* Étape 2 : libérer le bloc adjacent (ptr2) pour permettre une extension in-place */
    step("Étape 2: free(ptr2)  (rend le bloc adjacent libre)");
    free(ptr2);
    show_alloc_mem_ex();
    ...
    /* Sécurise l'affichage : on imprime 15 premiers caractères (index 0..14) */
    print_n_bytes_as_str("ptr1     = ", ptr1, 15);

    /* Étape 4 : réallocation de ptr1 vers 64 (idéalement in-place en fusionnant avec le bloc libre) */
    step("Étape 4: realloc(ptr1, 64)");
    old_ptr1 = ptr1;
    ptr_test = (char*)realloc(ptr1, 64);

    if (ptr_test) {
        /* Contenu conservé (min(15,64)=15). Affichons 15 premiers chars. */
